...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            # For double Q, we swap the critics: critic i uses target j's values
            # Roll the Q-values to swap them circularly
            next_qs = torch.roll(next_qs, shifts=1, dims=0)
        elif self.target_critic_backup_type == "min":
            # Clip Q: use the minimum Q-value across all critics
            next_qs = next_qs.min(dim=0, keepdim=False).values
        elif self.target_critic_backup_type == "mean":
            # Use the mean Q-value across all critics
            next_qs = next_qs.mean(dim=0, keepdim=False)
        else:
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # Use rsample for reparametrization trick (allows gradients through samples)
        ...
        # For a single sample estimate: -log_prob
        return -log_prob
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            # Need to expand obs to match action samples
            obs_expanded = obs.unsqueeze(0).expand(self.num_actor_samples, batch_size, -1)
            q_values = torch.stack([
                self.critic(obs_expanded[i], action[i])
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        # Note: Think about whether to use .rsample() or .sample() here...
        # Use rsample for reparametrization trick (allows gradients to flow through)
        action = action_distribution.rsample()

        # TODO(student): Compute Q-values for the sampled state-action pair
        q_values = self.critic(obs, action)
        # Take mean over critic ensemble
        q_values = q_values.mean(dim=0)

        # TODO(student): Compute the actor loss
        # For reparametrization, we want to maximize Q-values, so minimize negative Q
        loss = -q_values.mean()
