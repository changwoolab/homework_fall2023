...
class PGAgent(nn.Module):
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    # Compute TD error: δ_i = r_i + γ * V(s_{i+1}) * (1 - terminal) - V(s_i)
                    delta = rewards[i] + self.gamma * values[i + 1] * (1 - terminals[i]) - values[i]
                    # Compute GAE: A_i = δ_i + γλ * A_{i+1} * (1 - terminal)
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        return [sum(self.gamma ** i * reward for i, reward in enumerate(rewards)) for t in range(len(rewards))]
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        return [sum(self.gamma ** (i - t) * rewards[i] for i in range(t, len(rewards))) for t in range(len(rewards))]
