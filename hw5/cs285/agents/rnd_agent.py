...
class RNDAgent(DQNAgent):
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            # Get target and prediction from RND networks
            target = self.rnd_target_net(observations)
            prediction = self.rnd_net(observations)
            
            # Compute prediction error (RND bonus) - higher error means more novel/interesting state
            rnd_error = torch.norm(prediction - target, dim=-1)
            assert rnd_error.shape == rewards.shape
            
            # Add RND bonus to rewards
